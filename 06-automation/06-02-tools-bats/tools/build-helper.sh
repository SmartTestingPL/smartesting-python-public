...
function usage {
    echo "usage: $0: <download-editorconfigchecker|download-shellcheck|download-bats|install-zsd|generate-zsd|initialize-submodules>"
    exit 1
}
...
case $1 in
    download-editorconfigchecker)
        if [[ "${OSTYPE}" == linux* ]]; then
            EC_VERSION="2.1.0"
            EC_ARCHIVE="ec-linux-amd64.tar.gz"
            if [[ -x "${ROOT_DIR}/../build/editorconfigchecker/bin/ec" ]]; then
                echo "editorconfigchecker already downloaded - skipping..."
                exit 0
            fi
            wget -P "${ROOT_DIR}/../build/editorconfigchecker" \
                "https://github.com/editorconfig-checker/editorconfig-checker/releases/download/${EC_VERSION}/${EC_ARCHIVE}"
            pushd "${ROOT_DIR}/../build/editorconfigchecker"
            tar xzvf "${EC_ARCHIVE}"
            rm -vf -- "${EC_ARCHIVE}"
            ...
            echo "Please install editorconfig-checker manually:"
            echo "    https://github.com/editorconfig-checker/editorconfig-checker#installation"
            echo "    Place binary suitable for your OS under build/editorconfigchecker/bin/ec"
            exit 1
        fi
        ;;
    download-shellcheck)
